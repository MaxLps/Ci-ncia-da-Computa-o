...
void eliminacaoGaussThread(double** A, double b[]);
...
void eliminacaoGaussThread(double** A, double b[]) {
    ...
    for (j = 0; j < N; j++) {
        pivoteamento_parcial(A, b, j);
        #pragma omp parallel for private(i, k, m) shared(A, b) num_threads(NUM_THREADS)
        for (i = j + 1; i < N; i++) {
            m = -(A[i][j] / A[j][j]);
            for (k = j; k < N; k++) {
                A[i][k] = A[i][k] + (m * A[j][k]);
