#include <stdio.h>
#include <math.h>
void preenche_matriz(int n, double A[n][n], double I[n][n]){
    ...
    for(i=0; i<n; i++){
        for(j=0; j<n; j++){
            ...
            scanf("%lf",&A[i][j]);
            if(i==j){
                I[i][j] = 1;      //criando a matriz identidade para calcular a matriz inversa
            }
            else{
                I[i][j] = 0;
            }
...
void decomposicao_LU(int n, double A[n][n], double L[n][n], double U[n][n]){
    int k, i, j=0;
    double m;
    for(j=0; j<n; j++){
        for(i=j+1; i<n; i++){ 
...
void inversa(int n, double I[n][n], double L[n][n], double U[n][n]){
    double Ai[n][n], somatorio;  //Calculando a inversa de A 
    int i, j, k;                 //Ly = b(k) e Ux(k) = y, onde b(k) é a k-ésima coluna da identidade e x(k) é a k-ésima coluna da matriz invesa
    double y[n];
    for(k=0; k<n; k++){          //primeiro resolvemos Ly = b(k)
        y[0] = I[0][k];          //como a primeira linha de L é 1 0 0, podemos achar o valor de y1 diretamente com y1 = b(k)1 
        for(i=1; i<n; i++){
            somatorio = 0;
            for(j=0; j<i; j++){
                somatorio = somatorio + L[i][j]*y[j];  //Realizando as substiruições nas linhas de baixo para encontrar
            }                                          //os valores de y
            y[i] = (I[i][k]-somatorio)/L[i][i];
        }                                      //Resolvendo o sitema Ux = y
        Ai[n-1][k] = y[n-1] / U[n-1][n-1];    //Como U é triangular superior podemos achar o valor de x(k)n diretamente com x(k)n = yn/Unn
        for(i=n-2; i>=0; i--){
            somatorio=0;
            for(j=n-1; j>i; j--){
                somatorio = somatorio + (U[i][j] * Ai[j][k]);  //Realizando as substuiçoes sucessivas para achar os
            }                                                  //os valores de todas as incógnitas x(k)
            Ai[i][k] = (y[i] - somatorio)/U[i][i];
        }
    }
    printf("Inversa:\n");
    imprimir_matriz(n,Ai);
} 
int main (){
    int n;
    printf("Digite a ordem da Matriz\n");
    scanf("%d",&n);
    double A[n][n], L[n][n], U[n][n], I[n][n];
    printf("\nPreencha a matriz ampliada\n");
    preenche_matriz(n,A,I);
    imprimir_matriz(n,A);
